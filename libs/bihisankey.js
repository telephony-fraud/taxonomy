d3.biHiSankey = function () {
    ...
    links = linkGroups.map(function (linkGroup) {
      return linkGroup.reduce(function (previousLink, currentLink) {
        return {
          ...
          //"value": Math.min(5,previousLink.value + currentLink.value)
        };
      });
    });
  }
  ...
  function nodeHeight(sideLinks) {
    ...
    return Math.max(scaledValueSum + spacing, MIN_NODE_HEIGHT);
  }
  ...
  function computeNodeValues() {
    nodes.forEach(function (node) {
      ...
      node.height = Math.max(MIN_NODE_HEIGHT,Math.max(nodeHeight(visible(node.leftLinks)), nodeHeight(visible(node.rightLinks))));
  ...
  function compressInXDirection() {
    var connectedNodesXPositions,
        ...
        .key(function (node) {
	    debug_log("Node " + node.name + " value " + node.value +" x  pos " + node.x);
	    return node.x; })
  ...
  function computeNodeXPositions() {
    ...
    // nodes without successors gets x
    nodes.forEach(function(node){
  ...
  function computeNodeYPositions(iterations) {
    ...
    function calculateYScaleFactor() {
      ...
      yScaleFactor = d3.min(nodesByXPosition, function (nodes) {
        ...
        debug_log("discretionaryY="+discretionaryY+" nodeValueSum="+nodeValueSum+" height="+size[1])
        debug_log("discretionaryY/nodeValueSum="+discretionaryY/nodeValueSum)
	...
	nodesByXPosition.forEach(function (nodes) {
        nodes.forEach(function (node, i) {
          node.y = i;
            // we reserve space for the node that corresponds to the maximum size. size depends on the number of links
            // at max there are as many links as there are links comming from child nodes (complete expansion)
            node.heightAllowance = Math.max(
                MIN_NODE_HEIGHT,
                Math.max(
                    //nodeHeight(visible(node.leftLinks)),
                    //nodeHeight(visible(node.rightLinks))
                    nodeHeight(node.leftLinks.filter(function(link,i){return link.source.children.length === 0})),
                    nodeHeight(node.rightLinks.filter(function(link,i){return link.target.children.length === 0}))
                )
            );
        });
      });
    ...
    function showpos(mark){
        nodesByXPosition.forEach(function(nodes,i){
          if(i==0){
             nodes.forEach(function(node,j){
               debug_log(mark + " node ["+i+"]["+j+"].y=="+node.y)
              })
         }
      })
   }
    ...
    // compute a minimum for each column and adjust so that the first
    // node starts from the top
    nodesByXPosition.forEach(function(nodes){
  ...
  biHiSankey.connected = function (nodeA, nodeB) {
      //debug_log("nodeA="+nodeA+" nodeB="+nodeB+ " nodeA.connectedNodes="+nodeA.connectedNodes)
